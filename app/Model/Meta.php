...
class Meta
{
    ...
    private static function findCustomerRowByInviteField(
        ...
        // phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Column list is Customer::CUSTOMER_COLUMNS; table via %i; values are prepared.
        if ($json_field === 'token_hash' && $extra_args !== []) {
            $row = $wpdb->get_row(
                $wpdb->prepare(
                    'SELECT ' . Customer::CUSTOMER_COLUMNS . ' FROM %i
